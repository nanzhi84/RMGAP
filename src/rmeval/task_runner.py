...
class TaskRunner:
    ...
    def _load_dataset(
        ...
        for rec_idx, record in enumerate(raw_records):
            ...
            response_key_to_text: Dict[str, str] = {
                resp["key"]: resp["text"] for resp in responses
            }
    ...
    def _compute_consistency(
        ...
        return {"domain_group_to_rankings": domain_group_to_rankings}
    ...
    def _aggregate_metrics(
        ...
        for domain, groups in domain_group_to_rankings.items():
            flags: List[int] = []
            for _, rankings in groups.items():
                if not rankings:
                    continue
                first = rankings[0]
